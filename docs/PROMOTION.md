...
| 2026-09-11 | `basalPercentage`/`otherBasalPercentage`/`scheduledBasalsSum`/`correctionBolusPercentage`/`numOfCorrectionBolusesPerDay`/`automaticBolusPercentage`/`automaticBolusUnitsPerDay`/`automaticBolusCountPerDay` | stats (`get_basal_bolus_breakdown`, new tool) | Discovered via manual `field_capability` inspection | Same live-fetch-only situation as the CamAPS row above — these live in the same `data2` stats blob, none of them device-specific-prefixed (treated as a general Glooko stats capability, not CamAPS-only). |
| 2026-09-11 to 2026-09-15 | `pumpProfilesBasal[].segments` (`basalRateSchedule`/`scheduledDailyBasalUnits`), `bgCorrectionThresholdSegments`, `bgGoalLow`/`bgGoalHigh`, `cgmSettings.*AlertEnabled`/`*AlertLimit`, `basalSettings.activeBasalProgram` | settings (archived, `get_settings_history`) | Backwards-compatibility/QA sweep | A run of fixes across several sessions, each the same shape: a field structurally confirmed present in the archived `settings` JSON, sitting as an unread sibling of a field this project already extracted (e.g. `bgCorrectionThresholdSegments` sits next to `targetBgSegments`/`isfSegments`/`insulinToCarbRatioSegments` in the exact same `profilesBolus[0]` object). `activeBasalProgram` in particular was found via real-account QA to be populated on an account with NO `maxBasalRate` at all — the two are independent, not a primary/fallback pair. See `CHANGELOG.md`'s `[Unreleased]` section from this period for the full per-field detail; not repeated here field-by-field to keep this log from ballooning. |
| 2026-09-15 | `targetBgSegments[].valueLow`/`valueHigh` | settings (archived, `get_settings_history`) | Shape-only sweep of the real archive | Direct siblings of `targetBgSegments[].value`, same object, never extracted. Fixing the extraction also surfaced a real ingest bug: `getActiveSettings`'s mg/dL-source normalisation only ever converted `value`, so these two would have shipped in raw source units next to an already-normalised `value` for a mg/dL-account. Fixed both together. |
| 2026-09-16 | ~24 siblings of the 8 fields in the 2026-09-11 `get_basal_bolus_breakdown` row above (bolus overrides, dosing by delivery method pump/pen/manual, premixed-insulin figures, `hasPen`/`hasPump`/`hasEditedDoses`/`showAutoBolusStats`) | stats (`get_basal_bolus_breakdown`, expanded) | Full `field_capability` sweep (names only, never values) | Queried every confirmed-real `stats` field name ever recorded and diffed against what this project actually extracts. `hasPen`/`hasPump`/premixed-insulin fields suggest Glooko's stats blob is built for both pump AND pen (MDI) accounts — something this pump-centric project hadn't previously accounted for. Individual field names not repeated here; see `CHANGELOG.md`. |
| 2026-09-16 | `tenthPercentile`/`tewentyFifthPercentile`(sic)/`median`/`seventyFiftnPercentile`(sic)/`ninetiethPercentile`/`stdDev`/`averageBg`/`readingsPerDay`/`incompleteReadings`/`hasPrimeDeviceData` | stats (`get_glucose_distribution`, new tool) | Same full sweep as the row above | An AGP-report-style percentile band, genuinely distinct from this project's own archive-based TIR/stdDev/CV (own calc uses the patient's configured thresholds; this is Glooko's fixed server-side aggregate). Glooko's own raw field names carry two apparent typos (marked "sic" above) — kept verbatim as the internal lookup keys since that's what the real response contains; the tool's OUTPUT field names are spelled correctly. |
| 2026-09-16 | `carbsPerDay`/`carbEntriesPerDay`/`mealsPerDay`/`deviceCarbsPerDay`/`deviceCarbEntriesPerDay`/`deviceCarbSources` | stats (`get_meal_logging_stats`, new tool) | Same full sweep | Meal/carb-entry counts, distinct from the nutrition-MACRO fields (`caloriesPerMeal`/`proteinPerMeal`/`fatPerMeal`) found in the same sweep but deliberately NOT promoted — see the scope note below. |
| 2026-09-16 | `bloodPressureSources`/`bpReadings`/`bpStartDate`/`bpEndDate`, `averageSteps`/`exercises`/`exerciseSources`/`rawExerciseEventCount`/`stepSources`, `weightSources`/`weightUnit`, `caloriesPerMeal`/`proteinPerMeal`/`fatPerMeal`, `uniqueMedications` | stats | Deliberately NOT promoted | Same full sweep as the three rows above confirmed these are real and populated too, but they belong to Glooko's general wellness-tracking feature area (blood pressure, exercise, weight, full nutrition, medications) — a different product surface than the diabetes pump/CGM device data this project is scoped to. Recorded here so a future sweep doesn't waste time rediscovering that these are real; the decision not to promote them is deliberate, not an oversight. |
| 2026-09-16 | `averageDuration`, `manualSource` | stats | Deliberately NOT promoted | Confirmed real field names, but their meaning is genuinely unclear from the name alone (unlike everything else in this log, which could be reasoned about with at least a defensible best-effort reading) — no other sibling field to anchor an interpretation against. Left unpromoted rather than guessed at; revisit if a future sync or registry contribution clarifies what these actually mean. |
