...
| `get_settings_history` | Every pump setting change in force during a window: DIA, max basal rate, the active basal program/profile name, the programmed basal-rate schedule, an overall glucose goal range, the device's own CGM alarm thresholds, and the time-segmented target/ISF/carb-ratio/correction-threshold profiles. Max basal rate and the active basal program are independent fields, some accounts populate one but not the other. A target segment may also carry `valueLow`/`valueHigh` alongside `value` when the device reports a range rather than a single point. The basal-rate schedule is the *programmed* baseline (what the pump would run in manual mode), not what a closed-loop algorithm actually delivered, compare it against `get_daily_insulin`'s real figures rather than expecting a match. |
...
| `get_basal_bolus_breakdown` | A finer-grained basal/bolus split: scheduled versus other basal, correction-only versus fully automatic bolus delivery, bolus overrides, and dosing broken out by delivery method (pump/pen/manual) including premixed-insulin figures for MDI/pen accounts. Field names are confirmed real for an account that shows them; their exact meaning is this project's best-effort reading of Glooko's own naming, not independently verified against real values, cross-check against Glooko's own app if precision matters. Same live-call caveats as the CamAPS tool above. |
| `get_glucose_distribution` | Glooko's own AGP-report-style glucose percentile band (10th/25th/median/75th/90th) plus stdDev and averageBg. Not a duplicate of `get_diabetes_summary`'s own TIR/stdDev/CV, which are computed from the raw archive against your own configured thresholds — this is Glooko's separate server-side aggregate; a gap between the two is informative, not an error. Same live-call caveats as the CamAPS tool above. |
| `get_meal_logging_stats` | Glooko's own meal/carb-entry counts for the window: carbs, carb entries, and meals per day, plus the subset that came from the device itself versus manually logged. Same live-call caveats as the CamAPS tool above. |
