...
* **A shareable, privacy-guardrailed discovery report.** Running `node src/discover.js` builds a report of which fields your account's data actually populates and how often — with every real value replaced by a fixed, fabricated placeholder before it's ever displayed. `node src/submit-registry-entry.js` walks through reviewing that report, a mandatory typed confirmation, an independent second privacy scan, and (if you have the GitHub CLI installed) opens a pull request to contribute it to this project's `schema-registry/` — so future users of your same pump/CGM combo benefit from what your account already proved out, without your data ever leaving your machine.
...
* **`src/submit-registry-entry.js`** — the full contribution-submission sequence: display the report in full, a mandatory typed confirmation, an independent second privacy scan (deliberately not trusting `discover.js`'s own redaction logic), a content-integrity hash check, and (if `gh` is installed and authenticated) opening a PR to `schema-registry/`.
